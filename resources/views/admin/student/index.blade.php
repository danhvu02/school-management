...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Student List (Total: {{ $data['getRecord']->total() }})</h1>
          </div>
                  ...
                  <tbody>
                    @foreach ($data['getRecord'] as $value)
                        <tr>
                            ...
                            <td>
                                @if(!empty($value->getProfile()))
                                    {{ date(), strotime($value->date_of_birth) }}
                                @endif</td>
                            <td>{{ $value->caste }}</td>
                            ...
                            <td>{{ $value->height }}</td>
                            <td>{{ $value->weight }}</td>
                            <td>{{ ($value->status == 8) ? 'Active': 'Inactive' }}</td>
                            <td>{{date('d-m-Y H:i A', strtotime($value->created_at))}}</td>
                            <td style="min-width: 150px">
                                <a href="{{ route('student.edit', $value->id) }}" class="btn btn-primary btn-sm">Edit</a>
                                <a href="{{ route('student.delete', $value->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure?')">Delete</a>
                            </td>
                        </tr>
                    @endforeach
                  </tbody>
